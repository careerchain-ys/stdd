...
| 章 | 内容 | 適用 |
| --- | --- | --- |
| 1. 概要 | 機能の目的・スコープ・関連ドキュメント参照 | 常に |
| 2. 主要な設計判断 | この機能**特有**の判断（選択＋理由） | 任意（なければ章ごと省略） |
| 3. 画面項目定義 | UI × バリデーション × DB マッピング | 画面 feature は必須 / 非画面は省略 |
| 4. 処理ロジック | 集計・変換・ドメインルール・フロー | 常に（コア） |
...
- **目的**: （この機能が何を実現するか。REQUIREMENTS のユースケースと対応）
- **スコープ**: （対象 / 対象外）
