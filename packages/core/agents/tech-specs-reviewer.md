...
- [ ] §1.1 対応表に **要件ID 列**があり、`UC-<feature>-NN` が REQUIREMENTS §2.1 の `**ID**:` 行と一致している。§4.2 その他処理フローに `FL-<feature>-NN` が付与されている
...
- [ ] REQUIREMENTS §2.1 の全ユースケースが 1 行ずつ載っている（**対象ID `UC-<feature>-NN` 一致**・漏れなし）
- [ ] TECH_DESIGN §4.2 その他処理フローがある場合、全フローが載っている（**対象ID `FL-<feature>-NN` 一致**）
