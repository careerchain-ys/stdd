...
- 確信が持てない箇所は**要確認マーカー**（`**⚠️要確認**｜仮説: … ／確認: …`）を残す。逆生成でも「実装からこう読めた」という**仮説とセット**で置き、人間レビューで是非を確定したら除去する。構文は `documenting-requirements` の「要確認マーカー」を SSoT とする。
...
- 出力の**要確認マーカー**は「人間に確認すべき項目（仮説つき）」として後で潰す。
...
- テンプレ本体（`.claude/skills/*/templates/`）は壊さず、自プロジェクトの `docs/` 配下にコピーして特化するのが基本。
...
- 2 階層構造と方法論: `stdd-methodology.md` §3.1
- 開発フロー図: `stdd-methodology.md` §5
