...
新規プロジェクトには既存実装が無いので、**最初から順行**で進む。既存プロジェクトのような「遡行（実装から Spec を起こす）」フェーズは無い。

| | 新規（本ガイド） | 既存（[guide-for-existing-project](./guide-for-existing-project.md)） |
| --- | --- | --- |
| 向き | 最初から **順行**（Spec → Test → 実装） | 導入時だけ **遡行**（実装 → Spec）→ その後 順行 |
