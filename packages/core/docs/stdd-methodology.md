...
- **処理ロジック**: 集計式・変換・ドメインルール・トランザクション境界・複数テーブル横断の流れ（手順 / 擬似コード）
...
| **内容** | ユーザージャーニー、ビジネス目標 | 画面項目・処理ロジック・エラーハンドリング・非機能要件 |
...
3. ステークホルダーレビュー
        ↓
4. TECH_DESIGN.md 作成 (処理ロジック等) + TEST_PLAN.md 作成 (Test Strategy)
        ↓
5. アーキテクトレビュー
        ↓
