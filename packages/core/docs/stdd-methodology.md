...
- 仕様が正確であれば、AI エージェントはそれを唯一の入力として、Test Strategy に沿ったテスト作成から実装までを行い、機能性が担保された成果物を生み出せる
- 人間は仕様の正確さに集中すればよく、実装の品質は「仕様 → テスト → 実装」の連鎖で保証される
...
- UI / UX デザイン（HTML ワイヤーフレームへのリンク、表示要素）
  - 画面状態（通常 / 空 / ローディング / エラー）の設計は TECH_DESIGN.md に書く
  - ワイヤーフレームは `generating-wireframes` スキルで生成する
- 成功基準、スコープ外
...
- データ構造は `TABLE_DEFINITION.md`、API 契約は `API_SPEC.md` を参照する（再定義しない）
- テスト戦略は `TEST_PLAN.md` に分離する
...
- テストが正しく動作していることを証明する
- テストが実装の振る舞いを観察していることを保証する
