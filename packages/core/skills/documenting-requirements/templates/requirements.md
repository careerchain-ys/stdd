...
ユースケース（ユーザーが達成する単位）ごとに見出しを立て、直下に**トレーサビリティ ID**＋Priority を書き、
**振る舞い（手順）** と **受入基準・制約** を併記する。

**見出しは日本語の説明テキストのみ**（`UC1.` 等の連番を見出しに含めない）。安定 ID `UC-<feature>-NN`（`<feature>` は
feature slug、`NN` は連番）は見出し直下の `**ID**:` 行に置く。この ID を TECH_DESIGN・TEST_PLAN・テスト・実装へ
貫通させ、対応関係と抜け漏れを機械的に追跡する（→ 方法論「トレーサビリティ」）。**ID は名前と分離し、リネームしても不変**。

#### [ユースケース名]

**ID**: UC-<feature>-01 ／ **Priority**: P0
...
□ 全ユースケースに トレーサビリティ ID（見出し直下 `**ID**: UC-<feature>-NN`）＋Priority＋振る舞い（番号付き手順・主語明示）＋受入基準（EARS）がある
□ UC ID は feature 内で一意・連番。名前と分離され、リネーム前提で安定している（見出しには ID を含めない）
