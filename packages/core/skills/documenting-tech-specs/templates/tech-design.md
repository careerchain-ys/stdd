...
**要件ID（`UC-<feature>-NN`）は REQUIREMENTS §2.1 の見出し直下 `**ID**:` 行と一致させる**。この列がトレーサビリティ監査の
「設計リンク」になる（この表に無い UC は設計漏れとして検知される）。

| 要件ID | ユースケース（REQUIREMENTS §2.1） | Priority | 実現する設計 |
| --- | --- | --- | --- |
| UC-<feature>-01 | [ユースケース名] | P0 | §4.1 ユースケース別ロジック / §3 画面項目定義 |
| UC-<feature>-02 | [ユースケース名] | P1 | §4.1 ユースケース別ロジック |

### 1.2 使用 API

本機能が使用する API を列挙し、`API_SPEC.md` の定義へリンクする（API を持たない機能では本節を削除）。

| API | 用途 | 対応ユースケース ID | 定義 |
| --- | --- | --- | --- |
| `POST /api/...` | [用途] | UC-<feature>-01 | [API_SPEC](../../common/API_SPEC.md) |
...
REQUIREMENTS §2.1 の各ユースケースの実現ロジックを**ユースケース単位**で記述する（§1.1 の対応表〔要件ID〕と紐づく。各ユースケースが必ずロジックとして設計されていること）。見出しは説明テキストのみ。対応する要件ID は §1.1 の表で辿る。
...
ユースケースに直接紐づかない機能固有のロジック（バッチ / スケジュール処理・共通集計・内部整合処理 等）。各フローに識別名と**フローID `FL-<feature>-NN`**（FL の宣言はここが唯一の起点）を付け、TEST_PLAN の処理フロー別テストと対応づける。ID は見出し直下の `**ID**:` 行に置く（見出しは説明テキストのみ）。
