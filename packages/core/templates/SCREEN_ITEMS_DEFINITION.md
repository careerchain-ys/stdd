...
記述粒度の方針 (技術スタック非依存):
  - 言語固有の型 (TS型等) / ORM / フレームワーク詳細は記載しない (→ TECH_DESIGN.md)
  - DBマッピング (保存先テーブル・カラム) は画面項目と DB スキーマの対応を示す SSoT として記載する
  - 既存/新規カラムの別は別列を設けず、必要なら備考に注記する

含めない:
  - ユーザージャーニー / 画面遷移の文脈 (→ REQUIREMENTS.md)
  - アーキテクチャ / 実装コード / 言語固有の型 / RLS・インデックス等 DBメタ詳細 (→ TECH_DESIGN.md)
  - 関数名 / クラス名 / ファイルパス・行番号

書き換え方:
  プレースホルダ ([画面名], [SCREEN_ID], [項目名], [table], [column] 等) を実値に置き換え、
  画面タイプに合わない表・セクションは削除する。
  新規カラムも 1.画面項目定義 表の DBテーブル/DBカラム に記載し、別表で再掲しない (SSoTの二重化を避ける)。
-->
